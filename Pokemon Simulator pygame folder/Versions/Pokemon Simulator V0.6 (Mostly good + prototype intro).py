...
pokedexGraphic = pygame.image.load('Pokedex completed.png')
...
ballIndex = 0
index = 0
ballList = ["Poke Ball", "Master Ball"]
ballAmountList = [5, 999]
...
class pokemon(object):
  ...
  def catch(self, Pokemon2):
    ...
    if pokeCaught == True:
      #play victory music
      backgroundMusic = pygame.mixer.music.load('victory.mp3')
  ...
  def damageCalculation(self, Pokemon2, pokeMove, movePower, moveType, user):
    ...
    #Flailing  
    #elif flail == True:
      #statusTextUp = self.name.upper() + " struggled with life"
      #statusTextDown = ""
      #accuracy = 0
    # No status
    else:
  ...
  def battleVictory(self, Pokemon2):
    ...
    randNum /= 100
    prizeMoney = round(100 * Pokemon2.level * randNum)
    self.printBattleBG(Pokemon2)
...
def redrawGameWindow(background): 
  ...
  screenPrint("Press 'R' for RULES", 5, 375, (255, 255, 255), False, font2)
        ...
        if menuSelectD == True:
          ...
          if selectYes == True:
            ...
            while shopScreen == True:
              ...
              #if selectingItem == True and menuSelectD == True:
...
def rules():
  ...
  while ruleScreen == True:
    ...
    if menuSelectD == True:
      if menuScreen == rulesMenu:
        menuScreen = controlsMenu
      else:
        menuScreen = rulesMenu
...
offset = 0
offsetNum = 1
backgroundMusic = pygame.mixer.music.load('opening.mp3')
pygame.mixer.music.play(-1)
...
while run:
  ...
  if encounter == True:
    ...
    #wildPokemon = copy.deepcopy(Pikachu)
